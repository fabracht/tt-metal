...
void RunTestChipAndNocMCast(
    ...
    bool enable_fabric_tracing) {
    CoreCoord sender_logical_core = {0, 0};
    CoreRange receiver_logical_core_range = {{1, 0}, {1 + (mcast_rect_size_x - 1), mcast_rect_size_y - 1}};
    CoreCoord& receiver_start_logical_core = receiver_logical_core_range.start_coord;
    std::vector<tt_metal::Program> receiver_programs;
    ...
    chip_id_t src_phys_chip_id;
    std::unordered_map<RoutingDirection, std::vector<chip_id_t>> physical_end_device_ids_by_dir;
    int num_hops = start_distance + range - 1;
    ...
    auto last_recv_phys_chip_id = physical_end_device_ids_by_dir[dir][num_hops - 1];
    ...
    auto last_recv_fabric_node_id = end_fabric_node_ids_by_dir[dir][num_hops - 1];

    CoreCoord sender_virtual_core = sender_device->worker_core_from_logical_core(sender_logical_core);
    CoreCoord receiver_start_virtual_core =
        last_recv_device->worker_core_from_logical_core(receiver_start_logical_core);
    ...
    auto sender_kernel = tt_metal::CreateKernel(
        sender_program,
        "tests/tt_metal/tt_fabric/fabric_data_movement/kernels/tt_fabric_1d_mcast_tx.cpp",
        {sender_logical_core},
        tt_metal::DataMovementConfig{
            .processor = tt_metal::DataMovementProcessor::RISCV_0,
            .noc = tt_metal::NOC::RISCV_0_default,
    ...
    log_info(
        tt::LogTest,
        "Mcast Dst is from MeshId {} ChipId {} to MeshId {} ChipId {}",
    ...
    log_info(
        tt::LogTest,
        "Mcast Receiver Core Grid (Logical) is {},{} to {},{}",
        receiver_start_logical_core.x,
        receiver_start_logical_core.y,
        receiver_start_logical_core.x + mcast_rect_size_x - 1,
        receiver_start_logical_core.y + mcast_rect_size_y - 1);
    ...
    std::vector<uint32_t> sender_runtime_args = {
        ...
        start_distance,
        range,
        receiver_start_virtual_core.x,
        receiver_start_virtual_core.y,
        mcast_rect_size_x,
        mcast_rect_size_y};

    // append the EDM connection rt args for fwd connection
    chip_id_t dst_chip_id = first_recv_phys_chip_id;
    ...
    for (auto& [routing_direction, physical_end_device_ids] : physical_end_device_ids_by_dir) {
        for (auto physical_end_device_id : physical_end_device_ids) {
            ...
            tt_metal::SetRuntimeArgs(
                receiver_program, receiver_kernel, receiver_logical_core_range, receiver_runtime_args);
            fixture->RunProgramNonblocking(receiver_device, receiver_program);
            receiver_programs.push_back(std::move(receiver_program));
            log_info(tt::LogTest, "{} Rx Launched on physical device {}", routing_direction, physical_end_device_id);
        }
    ...
    for (auto [routing_direction, physical_end_device_ids] :
         physical_end_device_ids_by_dir) {  // lop over all cores in mcast grid!!!
        for (uint32_t i = 0; i < physical_end_device_ids.size(); i++) {
            auto* receiver_device = DevicePool::instance().get_active_device(physical_end_device_ids[i]);

            for (auto receiver_logical_core : receiver_logical_core_range) {
                log_info(
                    tt::LogTest,
...
// TEST_F(Fabric1DFixture, TestMCastConnAPIWithTracing) { RunTestMCastConnAPI(this, RoutingDirection::W, 3,
// RoutingDirection::E, 0, true); }
TEST_F(Fabric1DFixture, TestChipAndNocMCast) {
    RunTestChipAndNocMCast(this, RoutingDirection::E, true, false, 1, 3, 1, 1, true);
}  // only chip multicast (test start and range)
// TEST_F(Fabric1DFixture, TestChipAndNocMCast2) { RunTestChipAndNocMCast(this, RoutingDirection::E, true, false, 2, 2,
// 1, 1, true); } TEST_F(Fabric1DFixture, TestChipAndNocMCast3) { RunTestChipAndNocMCast(this, RoutingDirection::E,
// false, true, 3, 1, 4, 4, true); } // only noc multicast TEST_F(Fabric1DFixture, TestChipAndNocMCast4) {
// RunTestChipAndNocMCast(this, RoutingDirection::E, true, true, 1, 3, 4, 4, true); } // noc + chip multicast
