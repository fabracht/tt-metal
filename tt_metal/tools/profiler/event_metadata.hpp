...
struct alignas(uint64_t) KernelProfilerNocEventMetadata {
    ...
    static bool isFabricEventType(NocEventType noc_xfer_type) {
    ...
    static bool isFabricUnicastEventType(NocEventType noc_xfer_type) {
        return noc_xfer_type >= NocEventType::FABRIC_UNICAST_WRITE &&
               noc_xfer_type <= NocEventType::FABRIC_FUSED_UNICAST_ATOMIC_INC;
    }
